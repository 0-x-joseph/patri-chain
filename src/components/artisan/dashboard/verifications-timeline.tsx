        ...
        return (
            <div className="border border-slate-200 dark:border-slate-700 rounded-lg p-6">
                ...
                <div className="space-y-4">
                    {[...Array(4)].map((_, i) => (
                        <div key={i} className="flex gap-4 pb-4 border-b border-slate-200 dark:border-slate-700 last:border-0">
            ...
            {!verifications || verifications.length === 0 ? (
                ...
                <div className="space-y-4">
                    {verifications.map((verification, index) => (
                        <div
                            key={verification.id}
                            className="flex gap-4 pb-4 last:pb-0 border-b border-slate-200 dark:border-slate-700 last:border-0"
                        >
                            <div className="flex flex-col items-center">
                                <div className="w-3 h-3 bg-primary rounded-full flex-shrink-0"></div>
                                {index !== verifications.length - 1 && (
                                    <div className="w-0.5 h-12 bg-slate-200 dark:bg-slate-700 my-1"></div>
                                )}
                            </div>
